...
/***/ (function(module, exports, __webpack_require__) {
	...
	var utils = __webpack_require__(/*! privates/utils */ 1);
	var Events = __webpack_require__(/*! Events */ 7);
	var reporter = __webpack_require__(/*! privates/reporter */ 2);
	...
	var getBlob = function getBlob(type, namespace) {
		...
		var onResponseCallback = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : function () {};
	
		if (!utils.isObject(data)) {
			reporter.reportSdkMsg('Expecting params to be of type Object, ' + (typeof data === 'undefined' ? 'undefined' : _typeof(data)) + " given");
		}
	...
	!(__WEBPACK_AMD_DEFINE_RESULT__ = (function () {
		...
		var hasValue = function hasValue(obj, key) {
			return Boolean(obj) && isObject(obj) && hasOwnProperty.call(obj, key) && obj[key] !== undefined && obj[key] !== null;
		};
	...
	var errorMessages = {
		INVALID_ARG_TYPE_SHOULD_BE: function INVALID_ARG_TYPE_SHOULD_BE(argName, shouldBeType) {
			return 'Invalid argument - ' + argName + ' - should be of type ' + shouldBeType;
		},
		MISSING_MANDATORY_ARG_OF_TYPE: function MISSING_MANDATORY_ARG_OF_TYPE(argName, shouldBeType) {
			return 'Missing mandatory argument - ' + argName + ' - should be of type ' + shouldBeType;
		}
	};
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/responseHandlers */ 8), __webpack_require__(/*! privates/styles */ 9), __webpack_require__(/*! privates/core */ 6), __webpack_require__(/*! privates/viewMode */ 4), __webpack_require__(/*! privates/urlUtils */ 5)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, postMessage, reporter, responseHandlers, styles, core, viewMode, urlUtils) {
		'use strict';
		...
		var openMediaDialog = function openMediaDialog(messageType, namespace, supportedMediaTypes, mediaType, multipleSelection, onSuccess, onCancel, options) {
			...
			var callOnCancel = true;
	
			if (!utils.isFunction(onCancel)) {
				callOnCancel = false;
				if (!utils.isObject(options) && utils.isObject(onCancel)) {
					options = onCancel;
				}
			}
		...
		var getStyle = function getStyle(callback, key) {
			if (styles.Cache[key] && callback) {
				// TODO 08/08/2017 Adiel - core.addToReadyQ is now immediately invoking the callback if styles.Cache available so maybe we can remove that if block.
		...
		var getLocale = function getLocale(namespace) {
			postMessage.sendMessage(postMessage.MessageTypes.GET_LOCALE, namespace);
		...
		var getStateUrl = function getStateUrl(namespace, sectionId, state, callback) {
			if (!utils.isString(sectionId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('sectionId', 'String'));
				return;
			}
			if (!utils.isString(state)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('state', 'String'));
				return;
			}
			if (!utils.isFunction(callback)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('callback', 'Function'));
			}
		...
		var getCurrentPageId = function getCurrentPageId(namespace, callback) {
			if (!utils.isString(namespace)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('namespace', 'String'));
				return;
			}
		...
		var getCurrentPageNavigationInfo = function getCurrentPageNavigationInfo(namespace, callback) {
			if (!utils.isString(namespace)) {
			...
			postMessage.sendMessage(postMessage.MessageTypes.GET_CURRENT_PAGE_NAVIGATION_INFO, namespace, null, callback);
		};
		...
		var addApplicationImpl = function addApplicationImpl(namespace, appDefinitionId, options, onSuccess, onError) {
			if (!utils.isString(appDefinitionId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('appDefinitionId', 'String'));
				return;
			}
		...
		var isApplicationInstalled = function isApplicationInstalled(namespace, appDefinitionId, callback) {
			if (viewMode.getViewMode() === "site") {
				reporter.reportSdkError('Invalid view mode. This function can be called only in editor/preview mode.');
				return;
			}
			if (!utils.isString(appDefinitionId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('appDefinitionId', 'String'));
		...
		var addComponent = function addComponent(namespace, options, onSuccess, onError) {
			...
			if (options && options.appDefinitionId) {
				args.appDefinitionId = options.appDefinitionId;
				if (options.copyStyle && !options.styleId) {
					reporter.reportSdkError('Mandatory arguments - styleId must be passed when using copyStyle:true and adding a component of a different app');
					return;
				}
			}
			...
			if (options.componentType === 'PAGE') {
				...
				if (options.page.isHidden) {
					if (!utils.isBoolean(options.page.isHidden)) {
						reporter.reportSdkError('Invalid argument - isHidden should be of type boolean');
						return;
					}
				}
				...
				args.page = {
					...
					shouldNavigate: options.page.shouldNavigate,
					isHidden: options.page.isHidden
				};
			}
		...
		var isAppSectionInstalled = function isAppSectionInstalled(namespace, sectionId, options, callback) {
			if (!utils.isString(sectionId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('sectionId', 'String'));
				return;
			}
			...
			if (!utils.isFunction(callback)) {
				reporter.reportSdkError('Mandatory argument - callback function must be specified');
				return;
			}
			if (!utils.isObject(options)) {
			...
			postMessage.sendMessage(postMessage.MessageTypes.IS_APP_SECTION_INSTALLED, namespace, args, callback);
		};
		...
		return {
			...
			getCurrentPageNavigationInfo: getCurrentPageNavigationInfo,
			...
			isAppSectionInstalled: isAppSectionInstalled,
	...
	var postMessage = __webpack_require__(/*! privates/postMessage */ 0);
	var urlUtils = __webpack_require__(/*! privates/urlUtils */ 5);
	var currentEditMode;
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! Events */ 7), __webpack_require__(/*! privates/viewMode */ 4), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/urlUtils */ 5), __webpack_require__(/*! privates/styles */ 9)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, Events, viewMode, postMessage, urlUtils, styles) {
	...
	!(__WEBPACK_AMD_DEFINE_RESULT__ = (function () {
	  return {
		...
		/**
		 * Issued when the logged in member changes his personal details.
		 * @memberof Wix.Events
		 * @since 1.89.0
		 */
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! Error */ 14), __webpack_require__(/*! WixDataCursor */ 33)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Error, WixDataCursor) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! Events */ 7), __webpack_require__(/*! privates/templateUtils */ 31)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, utils, Events, templateUtils) {
		...
		var getFirstOrFallbackStyleParamValue = function getFirstOrFallbackStyleParamValue(match, p1) {
		...
		var mapSiteStyles = function mapSiteStyles(appStyle) {
			...
			styles.isVisualFocusEnabled = appStyle.isVisualFocusEnabled;
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! WindowPlacement */ 11), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (WindowPlacement, utils, reporter, postMessage, sharedAPI) {
		...
		var getDashboardAppUrl = function getDashboardAppUrl(options, callback) {
			...
			postMessage.sendMessage(postMessage.MessageTypes.GET_DASHBOARD_APP_URL, namespace, options, callback);
		};
		...
		var getCurrentPageNavigationInfo = function getCurrentPageNavigationInfo(callback) {
			sharedAPI.getCurrentPageNavigationInfo(namespace, callback);
		};
		...
		var addComponent = function addComponent(options, onSuccess, onError) {
			if (options && options.appDefinitionId) {
				delete options.appDefinitionId;
			}
			sharedAPI.addComponent(namespace, options, onSuccess, onError);
		};
		...
		var setFullWidth = function setFullWidth(shouldBeFullWidth, options, onSuccess, onError) {
			...
			if (options && options.margins) {
				...
				Object.keys(margins).forEach(function (key) {
					if (/%$/.test(margins[key])) {
						margins[key] = margins[key].replace('%', 'vw');
					}
				});
			}
		...
		var openLinkPanelImpl = function openLinkPanelImpl(options, onSuccess, onCancel) {
			if (!utils.isFunction(onSuccess)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('onSuccess', 'Function'));
				return;
			}
			if (onCancel && !utils.isFunction(onCancel)) {
				reporter.reportSdkError('Invalid argument - onCancel - should be of type Function');
				return;
			}
		...
		var openLinkPanel = function openLinkPanel(options, onSuccess, onCancel) {
			if (options && utils.isObject(options)) {
				openLinkPanelImpl(options, onSuccess, onCancel);
			} else if (utils.isFunction(options)) {
				onCancel = onSuccess;
				onSuccess = options;
				openLinkPanelImpl({}, onSuccess, onCancel);
			} else {
		...
		return {
			...
			/**
			 * The getCurrentPageNavigationInfo method returns the navigation info of the current page.
			 *
			 * @function
			 * @memberof Wix.Settings
			 * @since 1.91.0
			 * @author gileck@wix.com
			 * @example
			 *
			 * // The following call will return the an object containing the navigation info of the page.
			 * Wix.Settings.getCurrentPageNavigationInfo(function(linkData) {
			 *     //linkData contains:
			 *     ///"type" (could be "DynamicPageLink" or "PageLink"),
			 *     //"pageId" (exists only if the page is not a dynamic page),
			 *     //"routerId" AND "innerRoute" (exists only if the page is a dynamic page)
			 *     //store the "linkData" object to use it in Wix.NavigateTo(linkData)
			 * }
			 */
			getCurrentPageNavigationInfo: getCurrentPageNavigationInfo,
			 ...
			 * @since 1.90.0
			 * @param {Object} [options] can contain editorVersion which will result in returning appData.dashboardUrl or BizManager if experiment is open
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/utils */ 1)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, reporter, utils) {
		'use strict';
		...
		var refreshCurrentMember = function refreshCurrentMember(namespace) {
			postMessage.sendMessage(postMessage.MessageTypes.REFRESH_CURRENT_MEMBER, namespace);
		};
	
		return {
			getInstalledInstance: getInstalledInstance,
			refreshCurrentMember: refreshCurrentMember
		};
	}).apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__),
					__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, reporter, postMessage) {
	...
	* @namespace Wix
	*/
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! Theme */ 17), __webpack_require__(/*! WindowOrigin */ 13), __webpack_require__(/*! WindowPlacement */ 11), __webpack_require__(/*! privates/core */ 6), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/viewMode */ 4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, Theme, WindowOrigin, WindowPlacement, core, reporter, postMessage, sharedAPI, viewMode) {
		...
		var scrollTo = function scrollTo(x, y, options) {
			if (viewMode.getViewMode() === 'editor') {
				reporter.reportSdkError('Invalid view mode. This function cannot be called in editor mode. Supported view modes are: [preview, site]');
				return;
			}
			...
			if (options && options.hasOwnProperty("scrollAnimation")) {
				if (utils.isBoolean(options.scrollAnimation)) {
					args.scrollAnimation = options.scrollAnimation;
				} else {
					reporter.reportSdkError('Invalid argument - scrollAnimation should be of type boolean');
					return;
				}
			}
		...
		var navigateToComponent = function navigateToComponent(compId, options, onFailure) {
			...
			if (!utils.isString(compId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('compId', 'String'));
				return;
			}
		...
		var setPageMetadata = function setPageMetadata(options) {
			...
			if (!utils.isObject(options)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('options', 'Object'));
				return;
			}
			if (!utils.has(options, 'title') && !utils.has(options, 'description')) {
		...
		var requestLogin = function requestLogin(options, onSuccess, onCancel) {
			...
			if (utils.isFunction(options)) {
				...
				if (utils.isBoolean(options.checkCommunityCheckbox)) {
					args.checkCommunityCheckbox = options.checkCommunityCheckbox;
				}
				if (utils.isFunction(onSuccess)) {
					isCallOnSuccess = true;
				} else if (onSuccess) {
		...
		var navigateToAnchor = function navigateToAnchor(anchorId, onFailure) {
			...
			if (!utils.isString(anchorId)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('anchorId', 'String'));
				return;
			}
		...
		var isVisualFocusEnabled = function isVisualFocusEnabled(callback) {
			if (viewMode.getViewMode() !== 'site') {
				reporter.reportSdkError('Invalid view mode. This function cannot be called in editor and preview mode. Supported view mode is site');
				return;
			}
			...
			core.addToReadyQ(function (styles) {
				var isEnabled = !!(styles && styles.isVisualFocusEnabled);
				callback(isEnabled);
			});
		};
		...
		return {
			 ...
			 * @param [Object] options Can contain a 'scrollAnimation' property (boolean)
			 * @example
			 *
			 * Wix.scrollTo(0, 0);
			 *
			 ...
			 * @memberof Wix
			 * @since 1.92.0
			 * @param {Object} options object which contains the title and/or description properties to be set, empty value will reset them.
			 *
			 * @example
			 * Wix.setPageMetadata({
			 *      title: 'new title',
			 ...
			 *                           a 'language' property that define in which language the panel will be displayed in
			 *                           a 'checkCommunityCheckbox' which controls the community checkbox in the SignUp dialog
			...
			*/
			 ...
			 * @example
			 * // The following call will return the an object containing the navigation info of the page.
			 * Wix.getCurrentPageNavigationInfo(function(linkData) {
			 *     //linkData contains:
			 *     ///"type" (could be "DynamicPageLink" or "PageLink"),
			 *     //"pageId" (exists only if the page is not a dynamic page),
			 *     //"routerId" AND "innerRoute" (exists only if the page is a dynamic page)
			 ...
			 * );
			 */
			isApplicationInstalled: sharedAPI.isApplicationInstalled.bind(this, namespace),
	
			/**
			 * Checks if visualFocus turned on
			 * Available in site view mode only
			 *
			 * @function
			 * @memberOf Wix
			 * @since 1.87.0
			 * @param {function} callback called with the boolean result
			 *   callback signature: function(isEnabled) {}
			 * @example
			 *
			 * Wix.isVisualFocusEnabled(function (isEnabled) {
			...
			/**
			 * Checks if a section page installed by it's sectionId (aka - tpaPageId)
			 *
			 * @function
			 * @memberOf Wix
			 * @since 1.89.0
			 ...
			 *      can contain {string} appDefinitionId - app def id of the section
			 * @param {function} callback called with the boolean result
			 *   callback signature: function(isInstalled) {
			 *      console.log(isInstalled);
			 *   }
			 * @example
			 * var ECOM_APP_DEF_ID = '1380b703-ce81-ff05-f115-39571d94dfcd';
			 * Wix.isAppSectionInstalled('product_page', {appDefinitionId: ECOM_APP_DEF_ID}, (isInstalled)=>{});
			 */
			isAppSectionInstalled: sharedAPI.isAppSectionInstalled.bind(this, namespace)
		};
	}).apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__),
					__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	 ...
	 * @namespace Wix.Data
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/data */ 19)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, postMessage, reporter, sharedAPI, data) {
	
	  var namespace = 'Data.Public';
	
	  var getMulti = function getMulti() {
		data.getMulti.apply(data, [namespace].concat(Array.prototype.slice.call(arguments)));
	  };
	
	  var set = function set() {
		data.set.apply(data, [namespace].concat(Array.prototype.slice.call(arguments)));
	  };
	
	  var get = function get() {
		data.get.apply(data, [namespace].concat(Array.prototype.slice.call(arguments)));
	  };
	
	  var remove = function remove() {
		data.remove.apply(data, [namespace].concat(Array.prototype.slice.call(arguments)));
	  };
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/viewMode */ 4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, postMessage, reporter, viewMode) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/viewMode */ 4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, reporter, postMessage, viewMode) {
		...
		var registerCampaignPixel = function registerCampaignPixel(namespace, pixelType, pixelId) {
			...
			if (!utils.isString(pixelId) || !Number(pixelId) || pixelId === "") {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('pixelId', 'String, composes only digits, non empty'));
				return;
			}
		...
		var reportCampaignEvent = function reportCampaignEvent(namespace, eventType, data) {
			...
			for (var i = 0; i < requiredParams.length; i++) {
				if (!utils.isObject(data) || !utils.hasValue(data, requiredParams[i])) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, utils, reporter) {
		...
		var applicationLoadingStep = function applicationLoadingStep(namespace, stageNumber, stageDescription) {
			if (!utils.isNumber(stageNumber)) {
				reporter.reportSdkError(reporter.errorMessages.MISSING_MANDATORY_ARG_OF_TYPE('stageNumber', 'Number'));
				return;
			}
	...
	/* 24 */
	/*!********************************************!*\
	  !*** ./js/modules/privates/scriptUtils.js ***!
	  \********************************************/
	/*! dynamic exports provided */
	/*! all exports used */
	/***/ (function(module, exports, __webpack_require__) {
	...
	var scriptsSrc = {
		PM_RPC: 'https://unpkg.parastorage.com/pm-rpc@1.0.7/build/pm-rpc.min.js',
		PROMISE_POLYFILL: 'https://cdnjs.cloudflare.com/ajax/libs/es6-promise/4.1.1/es6-promise.auto.min.js'
	};
	
	var SCRIPT_ID_PREFIX = 'dynamic_script_';
	
	var loadScript = function loadScript(name, callback) {
		var existingElement = document.getElementById(SCRIPT_ID_PREFIX + name);
		if (existingElement) {
			if (callback) {
				callback();
			}
		...
		element.async = false;
		element.src = scriptsSrc[name];
		element.id = SCRIPT_ID_PREFIX + name;
	 ...
	 * @namespace Wix.Settings
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! Settings */ 10), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! superApps/privates/core */ 23), __webpack_require__(/*! superApps/privates/sharedAPI */ 12), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/utils */ 1)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Settings, postMessage, superAppsCore, superAppsSharedAPI, sharedAPI, reporter, utils) {
		'use strict';
		...
		var openMediaDialog = function openMediaDialog(mediaType, multipleSelection, onSuccess, onCancel, options) {
			sharedAPI.openMediaDialog(postMessage.MessageTypes.OPEN_MEDIA_DIALOG, namespace, this.MediaType, mediaType, multipleSelection, onSuccess, onCancel, options);
		};
		...
		var openDashboard = function openDashboard(options) {
			if (!utils.isNil(options) && !utils.isObject(options) || utils.isArray(options)) {
				reporter.reportSdkError(reporter.errorMessages.INVALID_ARG_TYPE_SHOULD_BE('options', 'Object'));
				return;
			}
	
			var params = utils.pick(options, ['state', 'origin']);
			var onCloseCallback = utils.has(options, 'onClose') ? options.onClose : null;
			if (!utils.isNil(params.state) && !utils.isString(params.state)) {
				reporter.reportSdkError(reporter.errorMessages.INVALID_ARG_TYPE_SHOULD_BE('options.state', 'String'));
				return;
			}
			if (!utils.isNil(params.origin) && !utils.isString(params.origin)) {
				reporter.reportSdkError(reporter.errorMessages.INVALID_ARG_TYPE_SHOULD_BE('options.origin', 'String'));
				return;
			}
			if (!utils.isNil(onCloseCallback) && !utils.isFunction(onCloseCallback)) {
				reporter.reportSdkError(reporter.errorMessages.INVALID_ARG_TYPE_SHOULD_BE('options.onClose', 'Function'));
				return;
			}
	
			postMessage.sendMessage(postMessage.MessageTypes.OPEN_DASHBOARD, namespace, params, onCloseCallback);
		};
		...
		return {
			 ...
			 * @since 1.92.0
			 * @experimental
			 * @param {Object} options Options for this function: appDefinitionId(string),  componentType (required), copyStyle (boolean), styleId (string), widget (object), page (object), appDefinitionId (string)
			 * @param {Function} onSuccess  Receives the component compId
			 * @param {Function} onError  Receives the error if the operation failed
			 * @example
			 *
			...
			*      console.log(compId);
			...
			/**
			 * Opens the Dashboard from the Settings panel. Use this method instead of openModal if your intention is to open
			 * your app's dashboard.
			 * @function
			 * @memberof Wix.Settings
			 * @author nirm@wix.com
			 * @since 1.92.0
			 * @param {Object} options Options for this function: state(String) - the state of the dashboard, maps to the hash. onClose(Function) - a callback to be called when Dashboard panel shuts. origin(String) - an optional parameter to determine the source of the call.
			 */
	...
	/* 26 */
	/*!********************************!*\
	  !*** multi ./js/WixPrivate.js ***!
	  \********************************/
	/*! dynamic exports provided */
	/*! all exports used */
	/***/ (function(module, exports, __webpack_require__) {
	
	module.exports = __webpack_require__(/*! ./js/WixPrivate.js */27);
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! ./Wix */ 28), __webpack_require__(/*! SuperApps */ 46), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/scriptUtils */ 24)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Wix, SuperApps, utils, scriptUtils) {
		"use strict";
	
		if (!window.Promise) {
			scriptUtils.loadScript(scriptUtils.scriptsName.PROMISE_POLYFILL);
		}
	...
	module.exports = global["Wix"] = __webpack_require__(/*! -!./node_modules/jshint-loader!./Wix.js */ 30);
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(/*! ./../node_modules/webpack/buildin/global.js */ 29)))
	...
	/* 30 */
	/*!************************************************!*\
	  !*** ./node_modules/jshint-loader!./js/Wix.js ***!
	  \************************************************/
	/*! dynamic exports provided */
	/*! all exports used */
	/***/ (function(module, exports, __webpack_require__) {
	
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/core */ 6), __webpack_require__(/*! Base */ 16), __webpack_require__(/*! Billing */ 32), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! Activities */ 34), __webpack_require__(/*! Settings */ 10),
			__webpack_require__(/*! Contacts */ 35), __webpack_require__(/*! Utils */ 36), __webpack_require__(/*! Styles */ 37), __webpack_require__(/*! Editor */ 38), __webpack_require__(/*! Events */ 7), __webpack_require__(/*! Error */ 14), __webpack_require__(/*! Media */ 15), __webpack_require__(/*! WindowOrigin */ 13),
			__webpack_require__(/*! WindowPlacement */ 11), __webpack_require__(/*! Worker */ 39), __webpack_require__(/*! PubSub */ 40), __webpack_require__(/*! Preview */ 41), __webpack_require__(/*! Dashboard */ 42), __webpack_require__(/*! Theme */ 17), __webpack_require__(/*! Features */ 43), __webpack_require__(/*! privates/urlUtils */ 5), __webpack_require__(/*! Data */ 18), __webpack_require__(/*! Analytics */ 44), __webpack_require__(/*! Performance */ 45)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (core, Base, Billing, utils, Activities, Settings,
		...
		var getDefaultNamespaces = function () {
			return {
				...
				getCurrentPageNavigationInfo: Base.getCurrentPageNavigationInfo,
				...
				isApplicationInstalled: Base.isApplicationInstalled,
				isAppSectionInstalled: Base.isAppSectionInstalled,
				isVisualFocusEnabled: Base.isVisualFocusEnabled
			};
		};
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/core */ 6), __webpack_require__(/*! privates/utils */ 1)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (core, utils) {
		...
		var evalTemplate = function evalTemplate(template, replaceCallback) {
			...
			return template.replace(/\{{2}([^}|^\{]*)\}{2}/gmi, replaceCallback); // eslint-disable-line
		};
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/responseHandlers */ 8), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, utils, responseHandlers, reporter, sharedAPI) {
	 ...
	 * @private
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage) {
		...
		var _callService = function _callService(onSuccess, onFailure, cursorId) {
			var onComplete = function onComplete(response) {
				if (response.error) {
					...
					this._nextCursor = response.data.nextCursor;
					this._previousCursor = response.data.previousCursor;
			...
			postMessage.sendMessage(this._serviceMessageType, namespace, args, onComplete.bind(this));
		};
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/responseHandlers */ 8), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/viewMode */ 4)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, responseHandlers, sharedAPI, reporter, viewMode) {
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/responseHandlers */ 8), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, responseHandlers, reporter, postMessage) {
	 ...
	 * @namespace Wix.Utils
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/core */ 6), __webpack_require__(/*! Media */ 15), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/urlUtils */ 5), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (core, Media, utils, reporter, urlUtils, postMessage, sharedAPI) {
		...
		var navigateToSection = function navigateToSection() {
			sharedAPI.navigateToSection.apply(sharedAPI, [namespace].concat(Array.prototype.slice.call(arguments)));
		};
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/styles */ 9), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, reporter, styles, utils, sharedAPI) {
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/reporter */ 2)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (sharedAPI, reporter) {
	
		var namespace = 'Editor';
	
		var isApplicationInstalled = function isApplicationInstalled(appDefinitionId, callback) {
			reporter.reportSdkMsg('Wix.Editor.isApplicationInstalled is DEPRECATED use Wix.isApplicationInstalled');
			sharedAPI.isApplicationInstalled(namespace, appDefinitionId, callback);
		};
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! Data */ 18), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/pubSub */ 20), __webpack_require__(/*! privates/data */ 19), __webpack_require__(/*! privates/analytics */ 21), __webpack_require__(/*! privates/performance */ 22)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Data, sharedAPI, postMessage, pubSub, data, analytics, performance) {
	  ...
	  var navigateToSection = function navigateToSection() {
		sharedAPI.navigateToSection.apply(sharedAPI, [utilsNamespace].concat(Array.prototype.slice.call(arguments)));
	  };
	  ...
	  return {
		 ...
		 * @example
		 * var ECOM_APP_DEF_ID = '1380b703-ce81-ff05-f115-39571d94dfcd';
		 * Wix.Worker.isAppSectionInstalled('product_page', {appDefinitionId: ECOM_APP_DEF_ID}, (isInstalled)=>{});
		 */
		isAppSectionInstalled: sharedAPI.isAppSectionInstalled.bind(this, namespace),
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/pubSub */ 20)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (pubSub) {
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! Base */ 16), __webpack_require__(/*! Settings */ 10), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/utils */ 1)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Base, Settings, reporter, postMessage, sharedAPI, utils) {
		...
		var navigateToDashboard = function navigateToDashboard() {
			postMessage.sendMessage(postMessage.MessageTypes.NAVIGATE_TO_DASHBOARD, namespace);
		};
		...
		return {
			...
			/**
			 * The Wix.Dashboard.navigateToDashboard method is to be used when your app is hosted as an iFrame inside Business Manager
			 * @function
			 * @since 1.89.0
			 * @memberof Wix.Dashboard
			 */
	 ...
	 * @namespace Analytics
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/viewMode */ 4), __webpack_require__(/*! privates/analytics */ 21)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, reporter, postMessage, viewMode, analytics) {
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/performance */ 22)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (performance) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! superApps/Settings */ 25), __webpack_require__(/*! superApps/privates/core */ 23), __webpack_require__(/*! superApps/Base */ 47), __webpack_require__(/*! superApps/Dashboard */ 48), __webpack_require__(/*! superApps/OnBoarding */ 53), __webpack_require__(/*! superApps/Billing */ 49), __webpack_require__(/*! superApps/Mobile */ 50), __webpack_require__(/*! superApps/Editor */ 51), __webpack_require__(/*! superApps/Worker */ 52)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (Settings, core, Base, Dashboard, OnBoarding, Billing, Mobile, Editor, Worker) {
		'use strict';
		...
		return {
			...
			getPublicAPI: Base.getPublicAPI,
			refreshCurrentMember: Base.refreshCurrentMember
		};
	}).apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__),
					__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	 ...
	 * @namespace Wix
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! superApps/privates/sharedAPI */ 12), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/viewMode */ 4), __webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/scriptUtils */ 24)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (sharedAPI, superAppsSharedAPI, reporter, viewMode, postMessage, utils, scriptUtils) {
		'use strict';
		...
		var refreshCurrentMember = function refreshCurrentMember() {
			superAppsSharedAPI.refreshCurrentMember(namespace);
		};
	
		var getPublicAPI = function getPublicAPI(options, onSuccess, onFailure) {
			if (!utils.isObject(options)) {
				reporter.reportSdkError('Mandatory argument - options must be specified');
				return;
			}
			if (!utils.isString(options.appDefinitionId)) {
				reporter.reportSdkError('Mandatory argument - options.appDefinitionId must be specified');
				return;
			}
			if (!utils.isFunction(onSuccess)) {
				reporter.reportSdkError('Mandatory argument - onSuccess function must be specified');
			...
			var callback = function callback(data) {
				if (data.error) {
					if (onFailure) {
						onFailure('Error - getPublicAPI of ' + options.appDefinitionId + ' appDefinitionId does not exist or app did not expose a public api');
					}
				} else {
					requestAPI();
				}
			...
			var requestAPI = function requestAPI() {
				sharedAPI.getCurrentPageId(namespace, function (currentPageId) {
					window.pmrpc.api.request('viewer_platform_public_api_' + options.appDefinitionId + '_' + currentPageId, { target: window.parent }).then(function (api) {
						onSuccess(api);
					}).catch(function () {
						if (onFailure) {
							onFailure('Error - getPublicAPI of ' + options.appDefinitionId + ' appDefinitionId does not exist or app did not expose a public api');
			...
			scriptUtils.loadScript(scriptUtils.scriptsName.PM_RPC, function () {
				postMessage.sendMessage(postMessage.MessageTypes.WAIT_FOR_WORKER_TO_BE_READY, namespace, null, callback);
			});
		};
		...
		return {
			...
			* }
			...
			/**
			 * This method is part of Wix Site Members feature. It allows the site to refresh the member details if they were updated somehow externally.
			 * @function
			 * @memberof Wix
			 * @since 1.90.0
			 * @example
			 *
			 * Wix.refreshCurrentMember();
			 */
			refreshCurrentMember: refreshCurrentMember,
	
			/**
			 * Returns the platform application public api
			 *
			 * Available in viewer, preview view modes.
			 * @function
			 * @memberof Wix
			 * @since 1.90.0
			 * @param {Object} options - can contain 'appDefinitionId' property (string)
			 * @param {Function} onSuccess - callback to be called with the returned functions
			 * @param {Function} [onFailure] - callback to be called on error
			 *
			 * @example
			 *
			 * Wix.getPublicAPI('appDefinitionId', onSuccess, onFailure);
			 */
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/postMessage */ 0), __webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! superApps/Settings */ 25)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (postMessage, utils, reporter, sharedAPI, superAppsSettings) {
		'use strict';
		...
		var openMediaDialog = function openMediaDialog(mediaType, multipleSelection, onSuccess, onCancel, options) {
			sharedAPI.openMediaDialog(postMessage.MessageTypes.SUPER_APPS_OPEN_MEDIA_DIALOG, namespace, superAppsSettings.MediaType, mediaType, multipleSelection, onSuccess, onCancel, options);
		};
	 ...
	 * @namespace Wix.Billing
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, reporter, sharedAPI) {
	 ...
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 1), __webpack_require__(/*! privates/sharedAPI */ 3), __webpack_require__(/*! privates/reporter */ 2), __webpack_require__(/*! privates/postMessage */ 0)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (utils, sharedAPI, reporter, postMessage) {
		'use strict';
	 ...
	 * @namespace Wix.Editor
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/sharedAPI */ 3)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (sharedAPI) {
	...
	/* 52 */
	/*!****************************************!*\
	  !*** ./js/modules/superApps/Worker.js ***!
	  \****************************************/
	/*! dynamic exports provided */
	/*! all exports used */
	/***/ (function(module, exports, __webpack_require__) {
	...
	/**
	 * A private namespace for the Worker of supperApps.
	 * @memberof Wix
	 * @namespace Wix.Worker
	 */
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! superApps/privates/sharedAPI */ 12)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (superAppsSharedAPI) {
		'use strict';
	
		var namespace = 'SuperApps.Worker';
